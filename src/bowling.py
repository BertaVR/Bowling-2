class Puntuacion_total_bolos:
    ...
    SPARE = '/'
    STRIKE = 'X'
    ####### máximo de bolos y máximo de frames 
    MAX_BOLOS = 10
    MAX_FRAMES = 10
    ...
    def __init__(self, tabla_tiradas):
        ...
        self.tiradas_por_frame = 0 #necesario para actualizar los frames, en las puntuaciones open y nulo será necesario para poder actualizar cuando llega el siguiente frametiradas_por_frame (cuando llegue a 2 sumará 1 frametiradas_por_frame)
        self.frames = 1 #este 1 lo pongo para que el frame especial sea el número 10 
    ...
    def puntuacion_partida(self):
        for tirada in self.tabla_tiradas: ###esta función recorre la tabla de tiradas y distribuye: le pasa cada puntuación de tirada a la función correspondiente (open, strike...)
            if self.frames < Puntuacion_total_bolos.MAX_FRAMES:
                if tirada.isdigit():
                ...
                ######FRAME ESPECIAL########## -Lo pasa a una función que simplemente sumará todo de ahí en adelante (tomando las equivalencias ente la puntuación y el símbolo). Cuando el frametiradas_por_frame sea el 10 (lo he "hackeado un poquito empezando los frames desde 1 y no desde 0")
            elif self.frames == Puntuacion_total_bolos.MAX_FRAMES:
    ...
    def puntuacion_open(self, tirada):
        self.tiradas_por_frame += 1 ##hemos hecho 1 tirada, la sumamos al frametiradas_por_frame (esto no lo hacemos en puntuacion_partida() pq por ejemplo los strikes suman diferente)
        self.puntuacion += int(tirada) ##sumar tirada
        if self.tiradas_por_frame == 2: ##cada vez que sea la segunda tirada del frametiradas_por_frame reiniciamos el contador de tiradas por frametiradas_por_frame y actualizamos frametiradas_por_frame sumando 1.
            self.frames += 1
            self.tiradas_por_frame = 0
        self.ultimo_numero = tirada

    def puntuacion_nulo(self, tirada):
        CERO = 0
        self.puntuacion += CERO ##si es nulo la puntuación es 0
        self.tiradas_por_frame += 1
        if self.tiradas_por_frame == 2: ##cada vez que sea la segunda tirada del frametiradas_por_frame reiniciamos el contador de tiradas por frametiradas_por_frame y actualizamos frametiradas_por_frame sumando 1.
            self.frames += 1
            self.tiradas_por_frame = 0 ## Aquí no va como constante el 0 porque es diferente, no es 0 por ser nulo, es 0 porque hay que reiniciar el contador cuando llega a 2 
    ...
    def puntuacion_spare(self):
        ...
        self.frames += 1  ##siempre que hay spare es en la segunda tirada, podemmos simplemete actualizar el frametiradas_por_frame y el contador de tiradas por frametiradas_por_frame
        self.tiradas_por_frame = 0  
    ...
    def puntuacion_strike(self):
        ...
        self.frames += 1 ##siempre que hay strike se salta de frametiradas_por_frame
        return Puntuacion_total_bolos.equivalencias_puntuacion_simbolo(self, lista_tiradas)
